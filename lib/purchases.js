    ...
    app.get('/purchases/:note_id', async (req, res) => {
        ...
        let result = await query(
            `WITH T AS (
                        ...
                        PC AS (
                            SELECT T4.product_id, CH.name, P_C.value FROM T4
                        ...
                        CHARACTERS AS (
                            ...
                            array_agg(value) as characters_values FROM PC
                            JOIN T4 ON T4.product_id = PC.product_id
                            GROUP BY PC.product_id
                            )
                            
                        SELECT T4.product_name, T4.product_logo, 
                            T4.price, T4.stock_price,
                            characters_names, characters_values,
                            T4.shop_name, T4.shop_logo
                            FROM T4
...
function group(rows) {
    ...
    for (let item in rows) {
        ...
        let characteristics = parseCharacteristics(item.characters_names, item.characters_values)
...
function parseCharacteristics(names, values) {
    let result = []
