    ...
    app.get('/purchases/:note_id', async (req, res) => {
        ...
        let result = await query(
            `WITH T AS (
                            ...
                            T2 AS (
                                SELECT prod_id, category_id, array_agg(purch_cat_char_value) as purch_values, array_agg(prod_char_value) as prod_values FROM T
                                GROUP BY prod_id, category_id
                            ),
                            PROD_IDS AS (SELECT prod_id, category_id FROM T2 WHERE purch_values = prod_values),
                            
                            T3 AS (SELECT category_id, MIN (COALESCE (stock_price, price)) as price FROM PROD_IDS
                            ...
                            T4 AS (
                                ...
                                FROM T3
                              LEFT JOIN products PP ON PP.category_id = T3.category_id
                                LEFT JOIN products_shops P_S ON P_S.product_id = PP._id AND (COALESCE (stock_price, P_S.price)) = T3.price
                                JOIN shops SH ON SH._id = P_S.shop_id
                                JOIN products PR ON PR._id = P_S.product_id
                            ),
                            
                            PC AS (SELECT T4.product_id, CH.name, E_V.value, CH.abbr FROM T4
                            ...
                            SELECT T4.product_name, T4.product_logo, 
                                         T4.price, T4.stock_price, PUR._id as purchase_id, PUR.count, PR.abbr as product_abbr,
...
function group(rows) {
    ...
    for (let item in rows) {
        ...
        let index = indexByShopName(result, item.shop_name)
